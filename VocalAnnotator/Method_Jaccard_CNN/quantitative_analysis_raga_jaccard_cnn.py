...
RAGA_NAME_MAPPING = {
    ...
    "Panthuvarali": "Pantuvarali",  # Fixed the 'h' mismatch
...
def predict_song_cnn(df, model, device, restrict_to_8=False):
    ...
    for note in SWARAS:
        ...
        for _, row in df.iterrows():
            try:
                # Use json.loads instead of ast.literal_eval for faster parsing if possible
        ...
        X = torch.tensor(np.array(extracted), dtype=torch.float32).unsqueeze(1).to(device)
        X = X / 50.0 
    ...
    # Dictionary Logic
    if restrict_to_8:
        # Search SPACE is limited to the dictionary-mapped names of our ground-truth set
        search_space = {RAGA_NAME_MAPPING[k]: MELAKARTA_RAGAS[RAGA_NAME_MAPPING[k]] for k in BASE_8_RAGAS}
    else:
        # Full 72 Melakarta Search
...
def main():
    print("="*60)
    print(" 🔍 QUANTITATIVE ANALYSIS: RAGA CLASSIFICATION BENCHMARK ")
    print("="*60)
    print("Methodology: Single-Tone Stability Modeling via 1D-CNN + Jaccard Matching")
    ...
    for folder_name in BASE_8_RAGAS:
        ...
        # Sample exactly 15 songs per raga to guarantee n=120
        ...
        for _, group_df in tqdm(eval_batch, desc=f"Evaluating {folder_name} "):
            ...
            # --- Experiment 1: Limited 8-Raga Search ---
            pred_8 = predict_song_cnn(group_df, model, device, restrict_to_8=True)
            if pred_8 == dict_raga_name:
                correct_base_8 += 1
                
            # --- Experiment 2: Full 72-Melakarta Search (Fine-Grained) ---
            pred_72 = predict_song_cnn(group_df, model, device, restrict_to_8=False)
            if pred_72 == dict_raga_name:
                correct_full_72 += 1
            
            # --- Chakra Check (Shared Purvanga) ---
            if pred_72 and dict_raga_name in MELAKARTA_RAGAS:
                # Comparison on lower tetrachord (Ri, Ga, Ma)
                if get_purvanga(MELAKARTA_RAGAS[pred_72]) == get_purvanga(MELAKARTA_RAGAS[dict_raga_name]):
                    correct_chakra += 1

    print("\n" + "="*60)
    print(" 📊 QUANTITATIVE DISCRIMINATION RESULTS ")
    print("="*60)
    print(f"Total Evaluated Dataset: n={total_songs} segments")
    
    if total_songs > 0:
        print(f"\n[Experiment 1: Limited Search Space (8 Ragas)]")
        print(f"• Top-1 Accuracy: {(correct_base_8/total_songs)*100:.1f}% ({correct_base_8}/{total_songs})")
        
        print(f"\n[Experiment 2: Full Search Space (72 Melakarta Ragas)]")
        ...
        print(f"\nBaseline (Random Guessing for n=72): 1.4%")
    print("="*60)

if __name__ == "__main__":
    # Small fix for the call to correct_chakra in the printer (previously missed parens?)
    # Wait, I'll fix that.
    main()
